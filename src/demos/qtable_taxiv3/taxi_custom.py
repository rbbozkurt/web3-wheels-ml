...
class customTaxiEnv(Env):
    ...
    * v0: Initial version release
    ...
    def custom_reset(self, passenger_location_index: int):
        self.s = categorical_sample(self.initial_state_distrib, self.np_random)
        self.lastaction = None
        self.taxi_orientation = 0

        # Optionally update passenger location
        if passenger_location_index is not None:
            # Decode the current state to get taxi row, taxi column, passenger index, and destination index
            taxi_row, taxi_col, passenger_idx, dest_idx = self.decode(self.s)

            # Encode the new state with the specified passenger location
            new_passenger_idx = passenger_location_index
            self.s = self.encode(0, 0, new_passenger_idx, 3)
